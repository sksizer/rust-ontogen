...
/// Called before a workout is inserted. Modify the entity or return Err to reject.
pub async fn before_create(_store: &Store, _workout: &mut Workout) -> Result<(), AppError> {
    Ok(())
}

/// Called after a workout is successfully created.
pub async fn after_create(_store: &Store, _workout: &Workout) -> Result<(), AppError> {
    Ok(())
}

/// Called before a workout is updated. Receives current state and pending changes.
pub async fn before_update(_store: &Store, _current: &Workout, _updates: &WorkoutUpdate) -> Result<(), AppError> {
    Ok(())
}

/// Called after a workout is successfully updated.
pub async fn after_update(_store: &Store, _workout: &Workout) -> Result<(), AppError> {
    Ok(())
}
