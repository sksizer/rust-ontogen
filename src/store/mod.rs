...
pub fn generate(
    ...
    for entity in entities {
        ...
        fs::write(&path, &code).map_err(|e| CodegenError::Store(format!("Failed to write {}: {e}", path.display())))?;
        crate::rustfmt(&path);
    ...
    fs::write(&path, &mod_rs).map_err(|e| CodegenError::Store(format!("Failed to write {}: {e}", path.display())))?;
    crate::rustfmt(&path);
...
fn generate_entity_store(entity: &EntityDef) -> String {
    ...
    code.push_str("use sea_orm::{ActiveModelTrait, EntityTrait};\n\n");
