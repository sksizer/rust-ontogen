...
",
    ...
    for m in modules {
        ...
        // Store-based modules (first param is &Store) get scoped routes only
        // (below) when route_prefix is set. Otherwise each store-scoped handler
        // opens the store through its surface's accessor.
