...
```
rust-ontogen/
├── crates/ontogen-core/   # Zero-dep crate: model types, IR structs, CodegenError, utilities
│   └── src/
...
├── crates/ontogen-macros/ # Proc-macro crate: #[derive(OntologyEntity)] (intentional no-op)
│   └── src/lib.rs
...
- **Severity:** medium
- **Location:** `crates/ontogen-core/src/ir.rs:188–200` vs. `src/servers/classify.rs:8–41`
...
- **Severity:** low
- **Location:** `crates/ontogen-core/src/lib.rs:29–41`
...
- **Observation:** `cruet = "1.0"` is listed as a direct dependency of the main crate, but grepping the source shows no `use cruet::` or `extern crate cruet` usage. All naming utilities (`to_snake_case`, `to_pascal_case`, `pluralize`) are in `crates/ontogen-core/src/naming.rs` and implemented directly.
