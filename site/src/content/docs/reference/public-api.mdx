...
| `config` | `&ClientsConfig` | Client configuration -- which client generators to run, `ts_skip_commands`, `schema_entities`, `pool_extra_roots`, etc. The `Pipeline` builder forwards `schema.entities` automatically. |

```rust
gen_clients(Some(&api), &[], &ClientsConfig {
    api_dir: "src/api/v1".into(),
    state_type: "AppState".into(),
    // ... shared with ServersConfig ...
    generators: vec![
