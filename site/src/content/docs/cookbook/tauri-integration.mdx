...
1. ### Add dependencies
   ...
   `specta` and `schemars` are dependencies of the generated DTOs (`Create{Entity}Input` / `Update{Entity}Input`) which still derive `specta::Type` and `JsonSchema`. The TypeScript bindings themselves no longer run through specta — see the [TypeScript bindings guide](/guides/typescript-bindings/) for how ontogen-ts handles type emission instead.
...
2. ### Write the build script
   ...
   fn main() {
       ...
       let clients_config = ClientsConfig {
           ...
           // Generated TS is emitted unformatted unless you set this.
