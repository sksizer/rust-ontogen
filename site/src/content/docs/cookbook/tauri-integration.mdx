...
1. ### Add dependencies
   ...
   [package]
   name = "my-app"
   default-run = "my-app"   # required once the TS bindings side-car lands
   ...
   `specta` and `schemars` are needed because the generated IPC commands use `#[specta::specta]` for TypeScript type generation and `JsonSchema` derives in DTOs. `specta-typescript` is what the TS bindings side-car uses to export your long-tail types -- see the [TypeScript HTTP client](/guides/client-generation/#the-bindings_path-option) section for details on when it's required.

   The `default-run` key matters once a custom (non-CRUD) API endpoint introduces a long-tail return type: ontogen then writes `src/bin/__ontogen_ts_export.rs`, which makes `cargo run` ambiguous between your app and the side-car. Setting `default-run` ahead of time avoids the surprise the first time `pnpm tauri dev` fails with "could not determine which binary to run."

2. ### Add `.taurignore` (Tauri-only)

   Tauri's dev-mode file watcher uses a custom `.taurignore` filename and does **not** honour standard `.gitignore`. Without an entry for the ontogen side-car file, the first `tauri dev` build will hot-reload itself in a loop on startup. Create `src-tauri/.taurignore`:
   ...
   Add the same line to `.gitignore` so the regenerated file doesn't show up in diffs. See [Integration gotchas](/guides/client-generation/#integration-gotchas) for the full set of side-car side-effects (including CI disk pressure on hosted runners).
