...
For Rust files, Ontogen formats in memory via `rustfmt` before comparing. TypeScript goes through whatever [`ts_formatter`](/reference/configuration/#tsformatter) you configured -- by default nothing, since Ontogen doesn't ship an opinion about your TS style. Either way the *formatted* output is what gets compared against disk, so formatting differences don't cause spurious writes either.
