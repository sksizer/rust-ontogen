...
```text
        ...
        { entity_tables, junction_tables, conversion_fns }
          ...
          └─────────────────┘    └─────────────────┘
              ServersOutput          () (TS bindings,
              { http_routes,         HTTP/IPC clients,
                ipc_commands,        admin registry)
                mcp_tools }
```

The servers and clients stages are siblings: both consume the same `ApiOutput` and neither produces input for the other. The pipeline runs them in stable order (`servers` then `clients`) for deterministic rerun-if-changed traces.
...
- **Full stack**: Chain everything through `gen_servers` for HTTP / IPC / MCP, plus `gen_clients` for TypeScript clients and the admin registry.
...
**`gen_servers()`** generates server-side transport handlers: Axum HTTP route handlers, Tauri IPC commands, and MCP tool definitions. Each transport reads the same `ApiOutput` and produces output appropriate for its protocol.

**`gen_clients()`** is `gen_servers`'s sibling for the client-side surface: TypeScript bindings, the HTTP-only and HTTP+IPC unified transport clients, and the admin-UI registry. It consumes the same `ApiOutput` but emits TypeScript instead of Rust.
