...
<Aside type="tip">
The [`Pipeline`](/concepts/pipeline/) builder constructs and runs `gen_servers` for you. Client-side TypeScript generation is a separate stage now -- see [Client Generation](/guides/client-generation/) for the matching `ClientsConfig` and `gen_clients` surface.
</Aside>
...
| `rustfmt_edition` | Rust edition used when formatting the generated Rust |
| `store_type` | Store type name for entity-scoped functions |
| `store_import` | Full import path for the Store type |
| `route_prefix` | Optional scope prepended to every route -- see [Route prefix](#route-prefix-for-project-scoping) |
| `pagination` | Optional pagination for list operations |

The full field list is in the [`ServersConfig` reference](/reference/configuration/#serversconfig).

<Aside type="note">
Client-side fields (`client_generators`, `ts_skip_commands`, `schema_entities`) used to live on this struct. They moved to [`ClientsConfig`](/reference/configuration/#clientsconfig) when client generation split into its own `gen_clients` entry point.
</Aside>
...
route_prefix: Some(ontogen::servers::RoutePrefix {
    // Colon-style here -- see the note below.
...
<Aside type="note" title="Colons in config, braces in output">
`RoutePrefix::segments` and the values in `sse_route_overrides` take **colon-style** parameters (`projects/:project_id`), while every generated axum route uses axum 0.8's **brace style** (`/api/projects/{project_id}/tasks`).

That asymmetry is deliberate: `axum_path()` normalizes each route string at emission, so build scripts written before the axum 0.8 cutover keep working untouched. Write colons in config; expect braces in the generated router and in `ServersOutput.http_routes`.
</Aside>
