...
<Aside type="caution">
When calling `gen_clients` directly (not through `Pipeline`), you **must** populate `ClientsConfig.schema_entities` if any of your generators is `AdminRegistry`. Without it, the admin-registry generator silently emits empty `fields: []` for every entity and the admin UI has nothing to render. `Pipeline::clients(...)` fills this in automatically.
</Aside>
...
`parse_schema` always comes first because every other generator needs `SchemaOutput.entities`. After that, `gen_seaorm`, `gen_markdown_io`, and `gen_dtos` are independent -- they only need the entity list. `gen_store` benefits from `SeaOrmOutput` but works without it. `gen_api`, `gen_servers`, and `gen_clients` form a chain where `gen_api`'s output feeds both `gen_servers` (Rust transports) and `gen_clients` (TypeScript clients + admin registry); the latter two are siblings and run in stable order for deterministic rerun-if-changed traces.
