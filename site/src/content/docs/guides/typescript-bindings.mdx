...
[`Pipeline`](/concepts/pipeline/) users get this filled in automatically from the `seaorm()` stage. Only direct `gen_clients` callers need to set it.
...
- `UnsupportedSerdeAttr` — a serde attribute isn't supported. Fix: drop the attribute, or use a workaround (split-rename → two fields + `From` impl). See [Not supported](#not-supported) for the current list.
...
<Aside type="note">
  Ontogen's downstream transport / client emitters (in `src/clients/generators/transport.rs` and `ts_client.rs`) carry their own fallback path that emits `Record<string, unknown>` for any type they can't find in `bindings.ts`. In the happy case `ontogen-ts` populates `bindings.ts` completely, so the fallback never fires. It exists as a defensive backstop against hand-edits to `bindings.ts` or root-set drift; if you see a `Record<string, unknown>` placeholder in the generated transport, it means the transport surface references a type `ontogen-ts` didn't include — usually a sign your `pool_extra_roots` or schema config needs adjusting.
</Aside>
