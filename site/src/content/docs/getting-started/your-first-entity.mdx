...
Let's wire up every generator. The recommended path is the [`Pipeline`](/concepts/pipeline/) builder -- one method call per stage, sensible defaults applied for you, and `schema.entities` is auto-forwarded to the admin-registry generator:
...
<Aside type="tip">
  Method order on `Pipeline` doesn't matter -- `build()` always runs stages in the canonical order (`schema → seaorm → markdown_io → dtos → store → api → servers`) and threads each stage's output into the next. Skip a method to skip a stage. The builder also auto-applies `schema_module_path` defaults so you don't have to repeat them on `StoreConfig` and `ApiConfig`.
</Aside>

If you'd rather see the explicit form (every generator function called by hand), that's covered in the [Build Script Setup](/guides/build-script-setup/) guide. Both shapes work identically -- `Pipeline` is just a wrapper.
