...
Four entity definitions produce 36 generated files. The entire backend -- persistence, CRUD, API, HTTP handlers, and IPC commands -- comes from those four annotated structs plus a `build.rs`.
...
From these four structs, `cargo build` produces 36 files across the project. Here's the breakdown.
...
../src-nuxt/app/generated/types.ts
../src-nuxt/app/admin/generated/admin-registry.ts
```
...
Iron Log's build script uses the `Pipeline` builder. The full file is around 85 lines; here it is, verbatim from `examples/iron-log/src-tauri/build.rs` minus one long explanatory comment:
...
Two fields worth noticing, because they're easy to miss and the compiler will demand them:

- **`ts_formatter`** — `TsFormatter::None` emits the TypeScript exactly as generated. Iron Log's Nuxt app runs its own formatter over the tree, so there's nothing for Ontogen to do here.
- **`schema_entities`** — left empty because `Pipeline` fills it from the schema stage. A direct `gen_clients` caller has to pass `schema.entities.clone()` or the admin registry ships with no field metadata.

Pipeline runs every stage in dependency order regardless of method-call order. It also threads each stage's typed output (`SchemaOutput → SeaOrmOutput → ApiOutput → ServersOutput / ()`) into the next, applies `schema_module_path` defaults to `StoreConfig`/`ApiConfig`, and forwards `schema.entities` into `ClientsConfig.schema_entities` so the admin-registry generator gets the field metadata it needs.
...
The Tauri build triggers `build.rs`, which runs the Ontogen pipeline and generates all 36 files before compilation. The Nuxt dev server picks up the generated TypeScript client.
